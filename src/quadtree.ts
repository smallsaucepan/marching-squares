...
class TreeNode {
  ...
  x: number;
  y: number;
  childA: TreeNode | null;
  childB: TreeNode | null;
  childC: TreeNode | null;
  childD: TreeNode | null;

  constructor(data: number[][], x: number, y: number, dx: number, dy: number) {
    let dx_tmp = dx,
      dy_tmp = dy,
      msb_x = 0,
      msb_y = 0;
  ...
  cellsBelowThreshold(threshold: number, subsumed: boolean | undefined) {
    let cells: Cell[] = [];
...
export { QuadTree, QuadTree as quadTree, TreeNode };
